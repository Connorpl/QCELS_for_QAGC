...
class RunAlgorithm:
    ...
    def get_result(self, seed: int, hamiltonian_directory: str) -> float:
        ...
        full_qb_ham = qp_hamiltonian.copy()    
        ham_constant = full_qb_ham.__getitem__(PauliLabel())
        if PauliLabel() in qp_hamiltonian:
            qp_hamiltonian.pop(PauliLabel())

        qp_hamiltonian = round_hamiltonian(qp_hamiltonian,num_pickup=ham_terms,coeff_cutoff=ham_cutoff)
        print("truncated ham size: ",qp_hamiltonian.n_terms)
        print("Number of fitting points: ", n_Z)
        ham_terms_trunc = qp_hamiltonian.n_terms
        nshots = 1e7//(2* (n_Z-1))

        print("shots per matrix element: ", nshots)
        hf_state = ComputationalBasisState(n_qubits, bits=2 ** n_elec - 1)
        controlled_trotter_step_circ = get_controlled_trotter_circuit(qp_hamiltonian,delta_t,n_qubits)

        print("N gate: ", len(controlled_trotter_step_circ.gates))
        print("dt: ",delta_t)

        fitting_points = np.zeros(n_Z, dtype=complex)
        ...
        try:
            for n in range(1,n_Z):
                fitting_points[n] = sample_a_circ(n, nshots,n_qubits,hf_state,controlled_trotter_step_circ,sampler)
                max_n=n

        except ExceededError as e:

            x_points = [delta_t*x for x in range(0,max_n)]
        ...
        x_points = [delta_t*x for x in range(0,n_Z)]
